...
// ─── Status Badge ──────────────────────────────────────────────────────────────
const STATUS_STYLES = {
    'Pending':     { bg: 'var(--blue-dim)',   color: 'var(--blue)',   border: 'var(--blue)' },
    'In Progress': { bg: 'var(--amber-dim)',  color: 'var(--amber)',  border: 'var(--amber)' },
    'Completed':   { bg: 'var(--green-dim)',  color: 'var(--green)',  border: 'var(--green)' },
    'Blocked':     { bg: 'var(--red-dim)',    color: 'var(--red)',    border: 'var(--red)' },
    'Skipped':     { bg: 'var(--bg3)',        color: 'var(--text3)',  border: 'var(--border2)' },
};
...
export function Toast({ message, type = 'success', onClose }) {
    ...
    return (
        <div onClick={onClose} style={{
            ...
            background: type === 'success' ? 'var(--primary)' : 'var(--red)',
            color: '#ffffff', borderRadius: 4, padding: '11px 22px',
            fontFamily: 'var(--mono)', fontSize: 12, zIndex: 200,
            maxWidth: '90vw', textAlign: 'center',
            boxShadow: 'var(--shadow-lg)', letterSpacing: '0.03em',
            cursor: 'pointer',
        }}>
            {message}
        </div>
...
export function Section({ title, children }) {
    return (
        <div style={{
            ...
            {title && (
                <div style={{
                    fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--primary)',
    ...
    return (
        <div style={{ gridColumn: full ? 'span 2' : undefined }}>
            <div style={{
                ...
                letterSpacing: '0.08em', marginBottom: 3, textTransform: 'uppercase',
            }}>{label}</div>
...
const BTN = {
    ...
    confirm: { background: 'var(--green-dim)', color: 'var(--green)', border: '1px solid var(--green)' },
    block:   { background: 'var(--red-dim)',   color: 'var(--red)',   border: '1px solid var(--red)' },
    ghost:   { background: 'transparent',      color: 'var(--text2)', border: '1px solid var(--border2)' },
    blue:    { background: 'var(--blue-dim)',  color: 'var(--blue)',  border: '1px solid var(--blue)' },
    amber:   { background: 'var(--amber-dim)', color: 'var(--amber)', border: '1px solid var(--amber)' },
};

export function Button({ variant = 'primary', onClick, disabled, children, fullWidth, style: extra = {} }) {
    const v = BTN[variant] ?? BTN.primary;
    return (
        <button onClick={onClick} disabled={disabled} style={{
            ...v,
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            gap: 8, padding: '10px 20px', borderRadius: 3,
...
export function Field({ label, required, error, children }) {
    return (
        <div style={{ marginBottom: 16 }}>
            ...
            {children}
            {error && (
                <div style={{ fontSize: 11, color: 'var(--red)', marginTop: 5, fontFamily: 'var(--mono)' }}>
                    {error}
                </div>
            )}
        </div>
...
export const inputStyle = {
    ...
    borderRadius: 3, padding: '10px 12px', fontFamily: 'var(--mono)',
    fontSize: 13, color: 'var(--text)', outline: 'none', transition: 'border-color .2s',
};

// ─── FreightChips ─────────────────────────────────────────────────────────────
export function FreightChips({ type, dir }) {
    const dirStyle = dir === 'Import'
        ...
        <span style={{ display: 'inline-flex', gap: 6, flexWrap: 'wrap' }}>
            {type && (
                <span style={{
                    ...
                    background: type === 'Sea Freight' ? 'var(--blue-dim)' : 'var(--purple-dim)',
            ...
            {dir && (
                <span style={{
                    fontSize: 10, fontFamily: 'var(--mono)', padding: '3px 8px',
                    borderRadius: 2, fontWeight: 500, ...dirStyle,
                }}>{dir}</span>
            )}
        </span>
    );
...
export function Modal({ title, onClose, children, footer }) {
    ...
    return (
        <div onClick={onClose} style={{
            position: 'fixed', inset: 0, background: 'rgba(0,88,53,0.18)',
            backdropFilter: 'blur(2px)', zIndex: 100,
            display: 'flex', alignItems: 'flex-end', justifyContent: 'center',
        }}>
            <div onClick={e => e.stopPropagation()} style={{
                background: 'var(--bg2)', border: '1px solid var(--border)',
                borderTop: '3px solid var(--primary)', borderRadius: '8px 8px 0 0',
                padding: '24px 20px', width: '100%', maxWidth: 480,
                boxShadow: 'var(--shadow-lg)',
            }}>
                <div style={{
                    fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 600,
                    color: 'var(--primary)', marginBottom: 16, letterSpacing: '0.04em',
                }}>{title}</div>
                {children}
                {footer && (
...
export function MetaTag({ children, primary, green, red, amber }) {
    return (
        <span style={{
            ...
            borderRadius: 2, fontWeight: 500,
            background: primary ? 'var(--primary-dim)' : green ? 'var(--green-dim)'
                       : red ? 'var(--red-dim)' : amber ? 'var(--amber-dim)' : 'var(--bg3)',
            color: primary ? 'var(--primary)' : green ? 'var(--green)'
                   : red ? 'var(--red)' : amber ? 'var(--amber)' : 'var(--text2)',
        }}>{children}</span>
    );
}
...
export function TaskCard({ task, onClick, clickable = true }) {
    return (
        <div
            ...
            style={{
                background: 'var(--bg2)', border: '1px solid var(--border)',
                borderLeft: '3px solid var(--border)', borderRadius: 'var(--radius)',
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
                    ...
                    <div style={{
                        ...
                        fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--primary)', fontWeight: 600,
                    }}>{task.sequence}</div>
                </div>
            </div>
            ...
            {task.order_customer && (
                <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 8 }}>
                    {task.order_customer}
                    {task.order_bl_no && (
                        <span style={{ color: 'var(--text3)', marginLeft: 8, fontFamily: 'var(--mono)', fontSize: 11 }}>
                            {task.order_bl_no}
                        </span>
                    )}
                    {task.order_ucr && (
                        <span style={{ color: 'var(--text3)', marginLeft: 8, fontFamily: 'var(--mono)', fontSize: 10 }}>
                            UCR: {task.order_ucr}
                        </span>
                    )}
                </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                {task.order_freight_type && (
                    <FreightChips type={task.order_freight_type} dir={task.order_direction} />
                )}
                {task.order_service_type && <MetaTag>{task.order_service_type}</MetaTag>}
                ...
                {task.assigned_to_name && !task.order_customer && <MetaTag>{task.assigned_to_name}</MetaTag>}
                {task.started_at   && <MetaTag amber>▶ {task.started_at.slice(0, 16)}</MetaTag>}
                {task.confirmed_at && <MetaTag green>✓ {task.confirmed_at.slice(0, 16)}</MetaTag>}
                {task.blocked_at   && <MetaTag red>✕ {task.blocked_at.slice(0, 16)}</MetaTag>}
            </div>
...
export function StatCard({ value, label, color = 'primary' }) {
    const colors = {
        primary: 'var(--primary)', green: 'var(--green)',
    ...
    return (
        <div style={{
            background: 'var(--bg2)', border: '1px solid var(--border)',
            borderTop: `3px solid ${c}`, borderRadius: 'var(--radius)',
            padding: '14px 8px', textAlign: 'center', boxShadow: 'var(--shadow)',
        }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 28, fontWeight: 700, color: c, lineHeight: 1 }}>
                {value}
            </div>
            <div style={{
